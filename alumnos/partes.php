...
include ($_SERVER["DOCUMENT_ROOT"]."/iesgn/includes/funciones.inc");
	...
	{	
			...
			$sql="insert into Partes (Ida,Tipo,Fecha,Comentario,Id_prof,Hora) values (".$_POST["id"].",'a','".cambiaf_a_mysql($_POST["fecha"])."','".$_POST["comentario"]."',".$_POST["profe"].",'".$_POST["hora"]."')";	
...
if($_GET) $ti=$_GET["tipo"]; else $ti=$_POST["tipo"];
	...
	{
		echo "<h1>Amonestación</h1><br>";
		echo "Alumno:<b>".$row["Nombre"]."</b> (".$row["Unidad"].")";
		echo "<form name=\"for\" action=\"partes.php\" method=\"post\">";
