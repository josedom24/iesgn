...
{		
	...
	$sql="select * from Alumnos,Partes where Alumnos.Id=Partes.Ida and Partes.Tipo='a' and Partes.Fecha='".cambiaf_a_mysql($_POST["fecha"])."' group by Partes.Ida order by Alumnos.Nombre";
	...
	echo "<center><h2>Resumen de amonestaciones</h2></center>";
	...
	while($row=mysql_fetch_array($result))
	{
		...
		echo "<td>".calcular_num_amonestaciones($row["Ida"])."/".calcular_num_sanciones($row["Ida"])."</td>";
		echo "</tr>";
...
if(!$_POST)
{
	echo "<form name=\"for\" action=\"resumen.php\" method=\"post\"  onsubmit=\"return validarF(this.fecha)\">";
	echo "Elige fecha del resumen de amonestaciones:<br>";
