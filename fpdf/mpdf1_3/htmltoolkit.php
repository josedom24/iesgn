...
* License:  GPL                                                                *
*                                                                              *
* Changes:	function blockAdjust() added                                       *
* 		fn. AdjustHTML edited to fix textarea bug					 *
*******************************************************************************/
...
function GetCodepage($llcc) {
	...
	else if ($lang == "ar") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "dejavusans,dejavusansB,dejavusansI,dejavusansBI,dejavusanscondensed,dejavusanscondensedB,dejavusanscondensedI,dejavusanscondensedBI,scheherazade"; $mpdf_directionality = "rtl";  $mpdf_jSpacing = "W"; }
	else if ($lang == "fa") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "dejavusans,dejavusansB,dejavusansI,dejavusansBI,dejavusanscondensed,dejavusanscondensedB,dejavusanscondensedI,dejavusanscondensedBI,scheherazade"; $mpdf_directionality = "rtl";  $mpdf_jSpacing = "W"; }

	else if ($lang == "ps") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "scheherazade"; $mpdf_directionality = "ltr"; $mpdf_jSpacing = "W";  }
	else if ($lang == "ur") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "scheherazade,freesans"; $mpdf_directionality = "rtl"; $mpdf_jSpacing = "W";  }

	// INDIC - only partial coverage
	else if ($lang == "as") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }	// Assamese
	else if ($lang == "bn") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "gu") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "hi") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "kn") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "ks") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "ml") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "ne") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }	// Nepal
	else if ($lang == "or") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "pa") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "sd") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }	// Sindhi
	else if ($lang == "ta") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freesans"; $mpdf_jSpacing = "W";  }
	else if ($lang == "te") { $mpdf_codepage = "UTF-8"; $mpdf_pdf_unifonts = "freeserif"; $mpdf_jSpacing = "W";  }
...
// =======================================================================================================
// Callback function from function printtablebuffer in mpdf
function tableRotate($type,$k,$y0,$x0,$w,$h,$r,$ph,$xadj,$a,$b,$c=0,$d=0,$e=0,$f=0) {
 if ($r > 0) {	// rotate +90 degrees clockwise
   $newx = $x0 + $xadj + $h - ((($ph - $b) - $y0));
   $newy = $ph - ($y0 + ($a - $x0));
	// TEXT
   if ($type == 'Td') { 	// xpos,ypos
	return 'BT 0 -1 1 0 '.sprintf('%.2f %.2f',$newx,$newy).' Tm'; 
   }
	// FILL
   else if ($type == 're') { 	// xpos,ypos,width,height
	return sprintf('%.2f %.2f %.2f %.2f',$newx,$newy,$d,-$c).' re'; 
   }
	// LINES
	...
	$newx2 = $newx + $diffy;
	$newy2 = $newy - $diffx;
	return sprintf('%.2f %.2f m %.2f %.2f',$newx,$newy,$newx2,$newy2).' l S'; 
   }
	// IMAGES
   else if ($type == 'img') { 	// width,height,xpos,ypos
   	$newx = $x0 + $xadj + $h - ((($ph - $d) - $y0));
   	$newy = $ph - ($y0 + ($c - $x0));
	return sprintf('q %.2f 0 0 %.2f %.2f %.2f',$b,$a,$newx,$newy).' cm 0 -1 1 0 0 0 cm /I'; 
   }
 }
 else {	// rotate -90 degrees clockwise
   $newx = $x0 + $xadj + (($ph - $b) - $y0);
   $newy = $ph - ($y0 + ($w - ($a - $x0)));
	// TEXT
   if ($type == 'Td') { 	// xpos,ypos
	return 'BT 0 1 -1 0 '.sprintf('%.2f %.2f',$newx,$newy).' Tm'; 
...
//returns an associative array (keys: R,G,B) from html code (e.g. #3FE5AA)
  ...
  if ( ($color{0} != '#') and ( stristr($color,'rgb') === false ) ) $color = $common_colors[strtolower($color)];
  ...
  else if (strstr($color,'(')) //case of RGB(r,g,b)
  {
...
function ConvertSize($size=5,$maxsize=0,$fontsize=false){
...
function AdjustHTML($html,$directionality='ltr',$usepre=true)
{
	...
	$html = preg_replace('/<([^!\/a-zA-Z])/iu','&lt;\\1',$html);	
	...
	if ($usepre) //used to keep \n on content inside <pre> and inside <textarea>
 	{
		...
		while($thereispre) //Recover <pre attributes>content</pre>
		{
			...
			$html = preg_replace('#<pre(.*?)>(.*?)</pre>#usi','<erp'.$temp[1][$iterator].'>'.$temp[2][$iterator].'</erp>',$html,1);
			$thereispre--;
			$iterator++;
		}
		$iterator = 0;
...
function GetSubstitutions($cp,$ufont='') {	// cp = codepage; ufont - specifiy font if unicode else returns empty array
 ...
 if (strtolower($cp) == 'utf-8') {
	...
	if (!$cw[32]) {	// Unicode font file does not include ASCII characters (Indic Raju scripts)
		for($i=32;$i<127;$i++) {
